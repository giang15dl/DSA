...
public:
    ...
    void addRestaurant(int mCityID, char mName[]) {
        mCityID--;
        restaurants.push_back(Restaurant(mName));
        hashName[mName] = R;
        cities[mCityID].addRestaurant(R);
        addHash(mName, R);
        R++;
    }
    ...
    int regionalValue(int mCityID, int mDist) {
        ...
        while (!q.empty()) {
            ...
            for (auto r : cities[u].res_indexes)
                topVal.push_back(restaurants[r].value);

            for (auto v : cities[u].neighbors) {
                if (visited[v])
                    continue;
